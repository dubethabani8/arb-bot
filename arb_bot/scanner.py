...
For each product in the watchlist:
  1. Fetch the Amazon sale price (what you can SELL it for via FBA)
  2. Fetch eBay sold price, Walmart price, and liquidation listings (where you might BUY it cheaper)
...
def check_opportunity(
    ...
    """
    Given buy and sell data for a product, calculate profit and return
    a deal dict if thresholds are met. Returns None otherwise.
    """
    buy_price = buy_data["price"]
    sell_price = sell_data["price"]

    # Sanity check: ignore if buy >= sell (obviously no margin)
    ...
    result = calculate_net_profit(
        ...
        shipping_to_fba=5.0,  # estimated inbound shipping to FBA warehouse
...
def scan_product(product: dict) -> list[dict]:
    ...
    # Step 1: Get the Amazon sale price (this is what we SELL for)
    ...
    # Step 2: Check eBay as a buy source
    ebay_data = get_ebay_sold_price(name)
    if ebay_data:
        ebay_price = ebay_data["price"]
        logger.info(f"    eBay sold price: ${ebay_price:.2f}")
        if min_buy <= ebay_price <= max_buy:
            deal = check_opportunity(product, ebay_data, amazon_data, "amazon")
            if deal:
                logger.info(f"    💰 DEAL: Buy eBay ${ebay_price:.2f} → Sell Amazon ${amazon_price:.2f} = ${deal['net_profit']:.2f} ({deal['roi_percent']:.1f}% ROI)")
                deals_found.append(deal)

    # Step 3: Check Walmart as a buy source
    ...
    if walmart_data:
        ...
        if min_buy <= walmart_price <= max_buy:
            ...
            if deal:
                logger.info(f"    💰 DEAL: Buy Walmart ${walmart_price:.2f} → Sell Amazon ${amazon_price:.2f} = ${deal['net_profit']:.2f} ({deal['roi_percent']:.1f}% ROI)")
                deals_found.append(deal)

    # Step 4: Check liquidation lots as a buy source
    liq_listings = get_liquidation_listings(name, max_results=3)
    for liq in liq_listings:
        liq_price = liq["price"]
        logger.info(f"    Liquidation lot: ${liq_price:.2f} (lot price — check qty)")
        # Note: liquidation prices are lot prices. You should manually verify
        # per-unit cost before acting on these alerts.
        if min_buy <= liq_price <= max_buy:
            deal = check_opportunity(product, liq, amazon_data, "amazon")
            if deal:
                deal["title"] += " [LOT — verify per-unit cost]"
                logger.info(f"    💰 DEAL (lot): ${liq_price:.2f} lot → Sell Amazon ${amazon_price:.2f}")
                deals_found.append(deal)
...
def run_scan():
    """
    Main scan function. Called by the scheduler every N minutes.
    Iterates through the entire watchlist and processes all deals found.
    """
    ensure_log_file()
    STATS["scans"] += 1
    ...
    # Process all deals found in this scan
    ...
    logger.info(f"Lifetime totals — Scans: {STATS['scans']} | Deals: {STATS['deals_found']} | Best ROI: {STATS['best_roi']:.1f}%\n")
